...
public class MinDia3203 {
    ...
    public static void main(String[] args) {
        ...
        //List<List<Integer>> edges1List = Arrays.stream(edges1).map(row->Arrays.stream(row).boxed().collect(Collectors.toList())).toList();
        //List<List<Integer>> edges2List = Arrays.stream(edges2).map(row->Arrays.stream(row).boxed().collect(Collectors.toList())).toList();
   ...
   static int findDiameter(ArrayList<ArrayList<Integer>> tree, int n) {
  ...
  static  void dfs(int node, int distance, ArrayList<ArrayList<Integer>> tree, boolean[] visited, int[] res) {
        visited[node] = true;
